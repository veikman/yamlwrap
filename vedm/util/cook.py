# -*- coding: utf-8 -*-
'''Text processing functions for source-controllable data.

These are concerned with formatting markup and the maintenance of raw text
files which are easier to edit, programmatically and manually, than the
database objects they can be refined into.
...
def all_md(model, fields):
    '''Convert incoming text from markdown to HTML.

    Called before a new object is saved in create()-like model methods.

    '''
    try:
        for field in model.cooked:
            if field.name in fields:
                fields[field.name] = md(fields[field.name])
        return fields
    except:
        s = 'Unable to convert markdown {} to HTML for {}.'
        logging.error(s.format(fields, model))
        raise


def md(raw):
    '''Convert markdown to HTML in a standardized way.'''

    raw = unwrap_paragraphs(raw)   # Join lines separated for ease of editing.
    extensions = ['markdown.extensions.footnotes',
                  'markdown.extensions.toc']
    return markdown.markdown(raw, extensions=extensions)


def cook(raw, model=None, order=True, unwrap=None, wrap=None, lint=None):
    '''Modify a serialized YAML string.
    ...
    cooked = pyaml.dump(data, **dump_args)
